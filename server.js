...
function enqueue(job) {
  if (queue.length >= MAX_QUEUE) {
    throw new Error('Queue sedang penuh, coba lagi beberapa menit.')
  }
  queue.push(job.id)
  updateJob(job.id, {
    status: 'queued',
...
async function processQueue() {
  ...
  updateJob(jobId, {
    status: 'running',
    progress: 7,
    message: 'Menyiapkan Chromium builder...'
  })
...
app.get('/api/health', (req, res) => {
  res.json({
    ok: true,
    service: 'webtoapk-railway-builder',
...
app.get('/api/download/:id/:type', (req, res) => {
  ...
  if (!job || job.status !== 'done') {
    return res.status(404).json({ status: false, message: 'File belum siap atau job tidak ditemukan' })
  }
...
app.listen(PORT, () => {
  console.log(`WebToAPK Builder running on :${PORT}`)
})
